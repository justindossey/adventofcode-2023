...
local function DummyModule(name)
	...
	function self.add_input(input) end

	function self.add_output(output) end

	function self.receive(pulse, input_obj) end
	return self
end
		...
		if pulse == "low" then
			if self.state == "off" then
				...
				for _, output in ipairs(self.outputs) do
					...
					table.insert(message_queue, { output = output.name, pulse = "high", sender = self.name })
				end
			else
				self.state = "off"
				for _, output in ipairs(self.outputs) do
					...
					inc_low_pulse_count()
					table.insert(message_queue, { output = output.name, pulse = "low", sender = self.name })
				end
			end
		end
	end
...
local function Conjunction(name)
	...
	function self.receive(pulse, input_obj)
		...
		if self.is_rx_conjunction and pulse == "high" and not cycles[input_obj.name] then
...
local function Broadcast(name)
	...
	function self.receive(pulse, input_obj)
		for _, output in pairs(self.outputs) do
			...
			table.insert(message_queue, { output = output.name, pulse = pulse, sender = self.name })
		end
	end
